...
// tddOverlay resolves "review" to ReviewPrompt and "review-fixer" to
...
func TestTddOverlayReviewStages(t *testing.T) {
	...
	fixer, ok := tddOverlay("review-fixer")
	if !ok {
...
func TestComposeStagePromptReview(t *testing.T) {
	...
	if strings.Contains(rev, tdd.TddDirToken) {
	...
	if strings.Contains(fix, tdd.TddDirToken) {
