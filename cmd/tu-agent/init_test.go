...
func TestInitSetupOnly_NoSkills(t *testing.T) {
	...
	// F7-C: prepare no longer materializes dev-flow agents — roles resolve to
	// embedded generic shells at runtime.
	if _, err := os.Stat(filepath.Join(dir, ".claude", "agents", "developer.md")); !os.IsNotExist(err) {
		t.Errorf("prepare must not materialize a developer agent, stat err=%v", err)
	}
...
func TestInitSetup_EmptyRepoWithLang(t *testing.T) {
	...
	// F7-C: prepare succeeds on an empty repo with --lang but materializes no
	// dev-flow agents (roles resolve to embedded generic shells).
	if _, err := os.Stat(filepath.Join(dir, ".claude", "agents", "developer.md")); !os.IsNotExist(err) {
		t.Errorf("prepare must not materialize agents, stat err=%v", err)
	}
}
