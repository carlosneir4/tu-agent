...
Run: `"$TU" prepare` (pass the path argument the user gave, if any).
...
(an OSS or knowledge-sharing repo), run `"$TU" prepare --public` — that commits a
...
already-initialized repo), run `"$TU" prepare --update` (same path argument). This
...
Run: `"$TU" prepare --augment-agents` (same path argument, if any).
